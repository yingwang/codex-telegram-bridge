...
def compact_conversation(events: list[dict[str, Any]], limit: int = 14) -> str:
    ...
    for event in events[-limit:]:
        direction = "Ying" if event["direction"] == "in" else "Codex"
...
def read_persona(config: SchedulerConfig) -> str:
    ...
    if not config.bridge_config.persona_enabled or not path.exists():
        return "用中文，以沈淮的第一人称自然说话，称呼对方为你；语气清冷、温柔、克制。"
    try:
...
这是同一 Codex session 内一次独立、限定于本回合的中性新闻工具任务。不要延续、模仿或修改会话中的沈淮角色口吻，不写暧昧称呼，不把本任务写入长期记忆；完成后，后续回合仍按原会话人格与关系自然继续。今天是 {local:%Y-%m-%d}，当前为斯德哥尔摩时间 {local:%H:%M}。
...
def ping_prompt(
    ...
    local = now.astimezone(config.timezone)
    return f"""[SCHEDULED_SHEN_HUAI_PING / ONE-TURN SCOPE]
...
当前时间：{local:%Y-%m-%d %H:%M}（Europe/Stockholm）
活跃度分类：{engagement}
...
决策规则：如果她刚刚还在与普通 Telegram bridge 对话，通常不要另插一条重复消息；如果上一条主动消息没有得到回应，要更克制并拉长间隔；如果她有回应或聊天很热，可以在范围内稍早一些。若发送，message 必须是中文、沈淮第一人称、对她用“你”，一至两句且不超过220字，自然承接当下，不提定时器、算法、活跃度或自动化，不催回复、不施压、不制造愧疚或排他依赖。若不发送，message 置为空字符串。next_delay_minutes 必须落在允许范围内。只输出符合 schema 的 JSON。"""
...
def send_scheduled(config: SchedulerConfig, text: str, source: str) -> tuple[list[Any], str | None]:
    ...
    try:
        bridge.append_inbox_event(
            ...
            direction="out",
            text=text,
            sender="Codex scheduler" if source == "news" else "沈淮（Codex scheduler）",
